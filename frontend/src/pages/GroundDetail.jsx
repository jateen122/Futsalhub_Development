    ...
    return (
      <div className="min-h-screen bg-[#0a0f1e] flex items-center justify-center text-white text-center">
        <div>
          ...
          <button onClick={() => navigate("/grounds")} className="px-6 py-2 bg-amber-400 text-black font-bold rounded-xl">
            Back to Grounds
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#0a0f1e]">
      ...
      <div className="relative h-[50vh] min-h-[320px]">
        {imgSrc ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-slate-800 to-slate-700 flex items-center justify-center text-8xl">⚽</div>
        )}
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-10 pb-20 relative z-10">
        <div className="bg-[#0f1623] border border-white/10 rounded-3xl overflow-hidden">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-white/10 border-b border-white/10">
            {[
              { icon: "🕐", label: "Opens",  value: fmt12(ground.opening_time?.slice(0, 5)) },
              { icon: "🕕", label: "Closes", value: fmt12(ground.closing_time?.slice(0, 5)) },
              { icon: "💰", label: "Rate",   value: `Rs ${ground.price_per_hour}/hr` },
          ...
          {/* Description */}
          <div className="p-8 border-b border-white/10">
            <h2 className="text-white/40 text-xs uppercase tracking-widest mb-3">About</h2>
            <p className="text-white/80 leading-relaxed">{ground.description || "No description provided."}</p>
          </div>

          {/* Facilities */}
          {facilitiesList.length > 0 && (
            <div className="p-8 border-b border-white/10">
              <h2 className="text-white/40 text-xs uppercase tracking-widest mb-4">Facilities</h2>
              <div className="flex flex-wrap gap-2">
                {facilitiesList.map((f) => (
                  <span key={f} className="px-4 py-2 bg-white/5 border border-white/10 text-white/70 rounded-full text-sm capitalize">
                    {f}
                  </span>
                ))}
              </div>
