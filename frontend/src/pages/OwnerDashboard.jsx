...
import {
  MapPin, Calendar, Clock, Bell, ChevronRight,
  Plus, BarChart2, CheckCircle, XCircle,
...
function StatCard({ label, value, icon, sub, accent }) {
  return (
    <div className={`relative bg-white rounded-2xl border p-5 shadow-sm hover:-translate-y-0.5 transition-transform overflow-hidden
      ${accent ? "border-amber-200" : "border-gray-200"}`}>
      {accent && <div className="absolute inset-0 bg-gradient-to-br from-amber-50 to-yellow-50 opacity-70" />}
      <div className="relative">
        ...
        <p className="text-sm font-semibold text-gray-400 uppercase tracking-wider mt-1">{label}</p>
...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setNotifs((notifData.notifications || notifData.results || notifData || []).slice(0, 5));
        setBookings(allB);
  ...
  const revenue = bookings
    .filter((b) => b.payment_received === true)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-amber-50 pt-20">
      <div className="w-full px-6 md:px-10 lg:px-14 xl:px-20 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <StatCard label="Total Bookings" value={bookings.length}  icon={<Calendar size={18} />} sub={`${todayBkgs.length} today`} />
          <StatCard label="Pending"        value={pending.length}   icon={<Clock size={18} />}    sub="awaiting action" />
          <StatCard label="Confirmed"      value={confirmed.length} icon={<CheckCircle size={18} />} />
          <StatCard
            label="Revenue"
            value={`Rs ${Math.round(revenue).toLocaleString()}`}
            icon={<DollarSign size={18} />}
            sub="Khalti payments received"
            accent
          />
        </div>
        ...
        {pending.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-amber-600 text-sm mt-0.5">Review and accept or decline below</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="relative h-48">
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    {/* Increased text size */}
                    <p className="text-white font-black text-xl leading-tight">{ground.name}</p>
                ...
                <div className="p-5">
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-gray-50 rounded-xl p-3 text-center border border-gray-100">
                      {/* Increased text size */}
                      <p className="text-xl font-black text-gray-900">Rs {ground.price_per_hour}</p>
                      <p className="text-xs text-gray-400 mt-0.5 font-medium">per hour</p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-3 text-center border border-gray-100">
                      <p className="text-xs font-bold text-gray-700">{fmt12(ground.opening_time)}</p>
                      <p className="text-[10px] text-gray-400">to</p>
                      <p className="text-xs font-bold text-gray-700">{fmt12(ground.closing_time)}</p>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              {[
                { label: "All Bookings",     sub: `${bookings.length} total`,        path: "/owner-bookings",   Icon: Calendar,  badge: pending.length },
                { label: "Pricing & Slots",  sub: "Dynamic pricing rules",            path: "/owner-pricing",    Icon: DollarSign },
                { label: "Analytics",        sub: "Revenue & booking trends",          path: "/owner-analytics",  Icon: BarChart2  },
                { label: "Notifications",    sub: unread > 0 ? `${unread} unread` : "All read", path: "/owner-notifications", Icon: Bell, badge: unread },
              ].map((a, i) => (
                <button key={a.path} onClick={() => navigate(a.path)}
                  ...
                  <div className="w-9 h-9 bg-gray-100 rounded-xl flex items-center justify-center group-hover:bg-amber-100 transition">
                    <a.Icon size={16} className="text-gray-400 group-hover:text-amber-600 transition" />
                  </div>
                  <div className="flex-1 min-w-0">
                    {/* Increased text size */}
                    <p className="font-semibold text-gray-800 text-base">{a.label}</p>
                  ...
                  <ChevronRight size={14} className="text-gray-300 group-hover:text-amber-500 transition flex-shrink-0" />
                </button>
              ))}
            </div>
          </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50 bg-amber-50/50">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-bold text-gray-900 text-base">Pending Requests</h2>
                    <span className="text-xs font-black bg-amber-500 text-white px-2 py-0.5 rounded-full">{pending.length}</span>
                  </div>
                  <button onClick={() => navigate("/owner-bookings")}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold text-gray-900 text-base">Today's Schedule</h2>
                  {todayBkgs.length > 0 && (
                ...
                <div className="divide-y divide-gray-50">
                  {todayBkgs.map((b) => (
                    <div key={b.id} className="flex items-center gap-4 px-6 py-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-900 text-sm truncate">{b.user_email}</p>
                        <p className="text-xs text-gray-400">{fmt12(b.start_time)} – {fmt12(b.end_time)}</p>
                      </div>
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-lg border capitalize ${STATUS_COLOR[b.status] || STATUS_COLOR.pending}`}>
                        {b.status}
                      </span>
                    </div>
                  ))}
                    ...
                    <div key={b.id} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition">
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 mt-0.5
                        ${b.status === "confirmed" ? "bg-emerald-400"
                          ...
                          : "bg-red-400"}`} />
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-900 text-sm truncate">{b.user_email}</p>
                        <p className="text-xs text-gray-400">{b.date} · {fmt12(b.start_time)}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        {b.payment_received ? (
                          <p className="text-sm font-bold text-emerald-700">Rs {b.total_price}</p>
                        ) : b.is_free_booking ? (
                          <p className="text-xs font-bold text-amber-600">Free</p>
                        ) : (
                          <p className="text-sm font-bold text-gray-500">Rs {b.total_price}</p>
                        )}
                        <span className={`text-[10px] font-bold capitalize
                          ${b.status === "confirmed" ? "text-emerald-600"
                            : b.status === "pending"  ? "text-amber-600"
                            : "text-red-500"}`}>
                          {b.status}{b.payment_received && b.status === "cancelled" ? " (paid)" : ""}
                        </span>
                      </div>
                    </div>
                  ))}
