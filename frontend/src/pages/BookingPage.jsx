...
import {
  Clock, MapPin, Calendar, IndianRupee, Gift, ChevronRight, Tag,
  AlertCircle, Ban,
} from "lucide-react";
...
function LoyaltyPanel({ groundId, useFree, onFreeToggle }) {
  ...
  const {
    confirmed_count, bookings_until_next_free, free_bookings_available,
    loyalty_threshold, progress_to_next_free,
  } = loyalty;
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-gray-900">Book 5, Get 1 Free</p>
          </div>
        </div>
        {confirmed_count > 0 && (
...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${BASE_URL}/api/bookings/ground/${ground.id}/booked-slots/?date=${selectedDate}`, {
        headers: { Authorization: `Bearer ${token}` },
      }).then((r) => (r.ok ? r.json() : { booked_slots: [] })),
      fetch(`${BASE_URL}/api/grounds/${ground.id}/blocked-slots/?date=${selectedDate}`)
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (useFree || activeToken) {
        const body = {
          ground: ground.id, date: selectedDate,
      ...
      const payRes = await fetch(`${BASE_URL}/api/payments/initiate/`, {
        ...
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          ground_id: ground.id, date: selectedDate,
          start_time: selectedSlot.start, end_time: selectedSlot.end,
              ...
              <div className="p-6">
                ...
                <div className="mt-3 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2">
                  <p className="text-blue-700 text-xs font-semibold flex items-center gap-1">
                    <Clock size={12} /> Booking must be made at least 30 minutes in advance
                  </p>
                </div>
                  ...
                  <div className="bg-red-50 border-2 border-red-300 rounded-2xl p-5 flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-red-700 text-sm mt-1">
                        {blockedInfo.block_reason ? `Reason: ${blockedInfo.block_reason}` : "The owner has closed this ground for the selected date."}
                      </p>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <div className="flex items-center gap-3 text-xs font-semibold flex-wrap justify-end">
                      ...
                      <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-blue-400 inline-block" />Off-Peak</span>
                      ...
                      <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-gray-300 inline-block" />Soon/Past</span>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                      {slots.map((slot) => {
                        ...
                        if (past) {
                          ...
                          topBadge = <span className="absolute -top-2 left-1/2 -translate-x-1/2 bg-orange-400 text-white text-[9px] px-2 py-0.5 rounded-full font-black">Too Soon</span>;
                          ...
                          btnClass = "bg-blue-50 border-blue-300 hover:border-blue-500 hover:bg-blue-100 cursor-pointer";
                          topBadge = <span className="absolute -top-2 left-1/2 -translate-x-1/2 bg-blue-400 text-white text-[9px] px-2 py-0.5 rounded-full font-black">Off-Peak 💰</span>;
                        } else {
                        ...
                        return (
                          <button key={slot.start} type="button" disabled={disabled}
                            ...
                            title={blocked ? blockInfo.reason || "Blocked by owner" : tooSoon ? "Must book at least 30 minutes in advance" : undefined}>
                            {topBadge}
                            ...
                            {!past && !tooSoon && !booked && !blocked && !blockedInfo.full_day && (
                              <div className={`text-[10px] font-black mt-1.5 ${
                                selected ? "text-white/80"
                                  ...
                                  : priceInfo.isOffPeak ? "text-blue-700"
                                  : "text-gray-400"}`}>
                            ...
                            {tooSoon && !past && (
                              <div className="text-[9px] text-orange-400 mt-1">30min required</div>
                            )}
                            {(blocked || blockedInfo.full_day) && !past && !tooSoon && (
                  ...
                  {hasPeakRules && (
                    <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center gap-2">
                      ...
                      <p className="text-amber-700 text-xs">
                        🔥 Peak hour slots have higher rates. Off-peak slots (💰) have discounted rates. Standard slots use the base price.
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  <div className={`rounded-2xl border-2 p-6 shadow-sm transition-all
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        {getSlotPriceInfo(selectedSlot).isOffPeak && !(useFree || activeToken) && (
                          <p className="text-blue-600 text-xs mt-1 font-semibold">
                            💰 {getSlotPriceInfo(selectedSlot).label || "Off-peak discount applied"}
                          </p>
                        )}
                      </div>
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    {selectedSlot && getSlotPriceInfo(selectedSlot).isPeak && !(useFree || activeToken) && (
                      <div className="flex justify-between items-center py-2 border-b border-gray-100">
                        ...
                        <span className="font-semibold text-amber-600">🔥 {getSlotPriceInfo(selectedSlot).label || "Peak Hour"}</span>
                      </div>
                    )}
                    {selectedSlot && getSlotPriceInfo(selectedSlot).isOffPeak && !(useFree || activeToken) && (
                      <div className="flex justify-between items-center py-2 border-b border-gray-100">
                        <span className="text-blue-600 font-semibold">Pricing</span>
                        <span className="font-semibold text-blue-600">💰 {getSlotPriceInfo(selectedSlot).label || "Off-Peak Discount"}</span>
                      </div>
                    )}
                  </div>
                    ...
                    {method === "khalti" && (
                      <div className="mt-3 bg-blue-50 border border-blue-200 rounded-xl p-3">
                        <p className="text-blue-700 text-xs">
                          ℹ️ Your slot is reserved only once Khalti payment completes. If you abandon payment, the slot remains open.
                        </p>
                      </div>
                    )}
                  </div>
