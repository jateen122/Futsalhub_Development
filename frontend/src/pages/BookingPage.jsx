        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-gray-900">
              Book {loyalty_threshold}, Get 1 Free
            </p>
          </div>
        </div>
        {confirmed_count > 0 && (
...
export default function BookingPage() {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // handleConfirm — KEY CHANGE:
  //
  // Khalti path: DO NOT create booking first. Pass ground/date/time directly
  //              to initiate_payment, which creates booking + Khalti session.
  //
  // Cash path:   Create booking first (via /api/bookings/create/), then record
  //              cash payment (same as before).
  //
  // Free / Token path: Create booking via /api/bookings/create/ (same as before).
  // ─────────────────────────────────────────────────────────────────────────
  const handleConfirm = async () => {
    ...
    try {
      ...
      // ── KHALTI payment ────────────────────────────────────────────────────
      // DO NOT create booking. Only initiate Khalti.
      // Booking is created in verify_payment() AFTER successful payment.
      ...
      // Redirect to Khalti
      // Slot remains FREE until payment completes!
                  ...
                  {loadingSlots ? (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                      {slots.map((slot) => {
                        ...
                        const disabled =
                          booked || past || blocked || blockedInfo.full_day;
                  ...
                  {hasPeakRules && (
                    <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center gap-2">
                      ...
                      <p className="text-amber-700 text-xs">
                        🔥 Peak hour slots have higher rates. Off-peak slots use
                        the base price.
                      </p>
                    </div>
                  )}
                    ...
                    {method === "khalti" && (
                      <div className="mt-3 bg-blue-50 border border-blue-200 rounded-xl p-3">
                        <p className="text-blue-700 text-xs">
                          ℹ️ Your slot will only be reserved once payment is
                          completed on Khalti. If you abandon payment, the slot
                          remains open.
                        </p>
                      </div>
                    )}
