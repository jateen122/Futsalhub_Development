...
export default function PlayerDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`${BASE_URL}/api/payments/`,            { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
    ])
  ...
  const stats = {
    ...
    spent:     payments.filter(p => p.status === "success").reduce((s, p) => s + parseFloat(p.amount || 0), 0),
  };
    ...
    <div className="min-h-screen bg-[#080d18] pt-20 pb-16">
      ...
      <div className="relative max-w-6xl mx-auto px-4">
        ...
        <div className="flex items-start justify-between mb-10">
          <div>
            <p className="text-white/30 text-sm uppercase tracking-[0.2em] mb-1">Welcome back</p>
            <h1 className="text-4xl font-black text-white capitalize">{firstName} <span className="text-amber-400">⚽</span></h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          ...
          {[
            { label: "Total Bookings", value: stats.total,     icon: "🏟️", color: "text-white",        accent: "from-white/5"    },
            { label: "Confirmed",      value: stats.confirmed, icon: "✅", color: "text-emerald-400",  accent: "from-emerald-500/10" },
            { label: "Pending",        value: stats.pending,   icon: "⏳", color: "text-amber-400",    accent: "from-amber-500/10"   },
            { label: "Total Spent",    value: `Rs ${stats.spent.toFixed(0)}`, icon: "💰", color: "text-amber-400", accent: "from-amber-500/10" },
          ].map((s) => (
            <div
              key={s.label}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          ...
          {[
            ...
            >
              ...
              <p className={`font-bold text-sm ${action.primary ? "text-black" : "text-white"}`}>{action.label}</p>
              <p className={`text-xs mt-0.5 ${action.primary ? "text-black/60" : "text-white/40"}`}>{action.desc}</p>
            </button>
          ))}
            ...
            {recentBookings.length === 0 ? (
              <div className="px-6 py-12 text-center">
                ...
                <button
                  ...
                  className="mt-4 px-5 py-2 bg-amber-400 text-black text-sm font-bold rounded-xl hover:bg-amber-300 transition"
                >
              ...
              <div className="divide-y divide-white/5">
                {recentBookings.map((b) => {
                  ...
                  return (
                    <div key={b.id} className="px-6 py-4 flex items-center gap-4 hover:bg-white/3 transition">
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="text-amber-400 font-bold text-sm">Rs {b.total_price}</p>
        ...
        <div className="mt-6 bg-white/3 border border-white/10 rounded-2xl p-6">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-white font-bold text-lg capitalize">{profile?.full_name || firstName}</p>
